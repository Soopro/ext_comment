...
(function() {
  var Ajax, InvalidRequestAPI, InvalidRequestData, InvalidRequestParam, InvalidRequestType, OPEN_ID_COOKIE_NAME, PROFILE_COOKIE_NAME, Q, ResouceNotFound, TOKEN_COOKIE_NAME, default_options, is_exports, procces_cookie_input, procces_cookie_output, root, supCookie, utils, version;
  ...
  root.SupExtComment = function(opts) {
    ...
    comment = {
      ...
      remove: function(params, success, failed) {
        var author_id, author_token, key;
        key = utils.url2key(params.key);
        author_id = supCookie.get(OPEN_ID_COOKIE_NAME);
        author_token = supCookie.get(TOKEN_COOKIE_NAME);
        if (author_id && author_token) {
          params.author_id = author_id;
